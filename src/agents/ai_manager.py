...
class AIManager:
    ...
    - Can disable reference agent per-move (for AI vs AI performance)

    Usage:
        manager = AIManager()

        # Human vs AI (with reference agent)
        result = manager.get_move(
            game_id="abc123",
            state=current_state,
            player_thinking_time=5.2,
            use_reference_agent=True
        )

        # AI vs AI (without reference agent faster)
        result = manager.get_move(
            game_id="abc123",
            state=current_state,
    ...
    def __init__(
        ...
        Args:
            ...
            enable_reference_agent: Whether to run reference agent (global setting)
    ...
    def get_move(
        ...
        use_reference_agent: Optional[bool] = None
    ) -> MoveResult:
        """
        ...
        Args:
            ...
            use_reference_agent: Override reference agent (optional)
                - None: Use global setting (self.enable_reference_agent)
                - True: Force enable reference agent
                - False: Force disable reference agent (for AI vs AI performance)
        ...
        # Determine if reference agent should run
        should_run_reference = (
            use_reference_agent
            if use_reference_agent is not None
            else self.enable_reference_agent
        )

        # Get move from reference agent (if enabled)
        reference_result = None
        if should_run_reference:
            # Ensure reference agent exists
            if game_id not in self.reference_agents:
            ...
            logger.debug(
                f"Game {game_id[:8]}: Reference agent skipped "
                f"(disabled for this move)"
            )
